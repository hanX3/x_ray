...
#define AGXRAYCOUNT 4000
#define AGXRAYSIGMA 3.0
...
void ModifiedDetectorEff(double par[3], double par1[10], double par2[10], double par3[10], vector<double> &y, vector<double> &yy);
void GetPeakInfo(double par[3], vector<double> &v_x, vector<double> &v_y, map<double, double> &m_p);
void CaliDetectorEff(double par[3], double par1[10], double par2[10], double par3[10], map<double, double> &m_p, map<double, double> &m_pp);
...
void analysis_test()
{
  ...
  vector<double> v_y;
  char file_name[1024];
  sprintf(file_name, "../spectrum/background-15KV-0.02mA.mca");
  ...
  vector<double> v_y_ae;
  ModifiedAttenEff(par_cali, v_y, v_y_ae);
  vector<double> v_y_ae_de;
  ModifiedDetectorEff(par_cali, par1_eff, par2_eff, par3_eff, v_y_ae, v_y_ae_de);
  /*
...
void RejectBackground(vector<double> &x, vector<double> &y, vector<double> &yy)
{
  ...
  sprintf(file_name_back, "../data/empty.mca");
...
void ModifiedDetectorEff(double par[3], double par1[10], double par2[10], double par3[10], vector<double> &y, vector<double> &yy)
{
  ...
  //y: peak info from mca spectrum 
  //yy: peak info from mca spectrum after eff modified
  double eff = 0.;
  double e = 0.;
  int bin_number = y.size();
  for(int i=0;i<bin_number;i++){
    ...
    for(int i=0;i<10;i++){
      if(e<EFFENERGY1){
        eff += par1[i]*pow(e, (double)i);
      }
      else if(e>EFFENERGY2){
        eff += par3[i]*pow(e, (double)i);
      }else{
        eff += par2[i]*pow(e, (double)i);
      }
    }
    yy[i] = y[i]/eff;
  }
...
#ifdef DRAW
  TCanvas *cc_mde = new TCanvas("cc_mde", "cc_mde", 0, 800, 480, 360);
  TH1D *h_mde = new TH1D("h_mde", "h_atten_dec_eff", bin_number, par[0], par[0]+par[1]*(bin_number-1)+par[2]*(bin_number-1)*(bin_number-1));
  for(int i=0;i<bin_number;i++){
    h_mde->SetBinContent(i, yy[i]);
  }
  cc_mde->cd();
  h_mde->Draw();
#endif
...
void GetPeakInfo(double par[3], vector<double> &x, vector<double> &y, map<double, double> &m_p)
{
  ...
  //test
  TH1D* h_test = new TH1D("h_test", "back", bin_number, x[0], x[bin_number-1]);
  double source[bin_number];
  for(int i=0; i<bin_number;i++) source[i] = h0_gpi->GetBinContent(i+1);
  spec->Background(source, bin_number, 10, TSpectrum::kBackDecreasingWindow, TSpectrum::kBackOrder2, kFALSE, TSpectrum::kBackSmoothing3,kFALSE);
  for(int i=0; i<bin_number;i++) h_test->SetBinContent(i+1, source[i]);

  for(int i=0; i<bin_number;i++) y[i]=source[i];
  ...
  for(int i=0;i<nfound;i++){
    ...
    double area = h0_gpi->Integral(x_left_stop, x_right_stop);
